<template>
  <div class="search">
    ...
    <div class="article-wrapper" v-infinite-scroll="loadMore"
         ...
         ref="searchWrapper">
      <div class="article-list" v-for="article,index in searchList">
        <div class="article-cell"
             v-if="(index+1)%5 === 1 || (index+1)%5 === 2 || (index+1)%5 === 3 || (index+1)%5 === 4">
          <a @click.stop.prevent="toArticleDetail(article.id)">
            <div class="article-left">
              <h1 v-html="$options.filters.highlight(article.title, keyWord)"></h1>
  ...
  export default{
    ...
    methods: {
      _initSearchList () {
        ...
        url = api['search'] + '?key_word=' + encodeURIComponent(this.keyWord) + '&per_page=' + this.pageSize + '&cur_page=' + this.currentPage
      ...
      toArticleDetail (articleId) {
        ...
        this.$router.push({
          path: '/search/articleDetail/' + articleId,
          query: {
      ...
      _touchEvent () {
        touch.touchX(this.$refs.searchWrapper, () => {
          this.$router.push('/')
        })
      },
      loadMore () {
    ...
    watch: {
      $route (to, from) {
        if (to.path === from.path) {
          this.searchList = []
          this.currentPage = 1
          this.busy = false        // 下拉是否允许 false可以下拉  true禁止
          this.isLoading = true
          this.noMoreInfo = false
